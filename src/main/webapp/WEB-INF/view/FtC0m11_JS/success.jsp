...
function WhenKeyInsert(slotID)
{
	...
	{
		document.getElementById("pageDetailCtrl_div").innerHTML += "<input type='text' id='isLogin_" + slotID + "' name='isLogin_" + slotID + "' value='0' >";
	}
...
{
	var elementID = event.srcElement.id;  //在IE下获取所点击的标签的id信息

	var val = document.getElementById(elementID).value;  //所点击的id的所得值
	//initToken_div层TN检查
	if( "label" == elementID )  //初始化令牌事填写的令牌标签名称
	{
	...
	//modifyTN_div层TN检查
	else if( "new_TN" == elementID )   //修改令牌名称时填写的新的令牌标签名称
	{
		if( val.length <= 32 && 0 != val.Trim().length )
		{
...
function CheckPINLength(v)
{
	...
	//modifyUPIN_div层（修改用户密码层）PIN码检查
	...
	//modifyOPIN_div层（修改管理员密码层）PIN码检查
	...
	//unblockToken_div层（令牌解锁层）PIN码检查
...
function showSlotList(flag)
{
	...
	//pageCtrl_div层 （usbkey卡的数量，以及当前卡槽信息：第一行数据）
	document.getElementById("pageCtrl_div").innerHTML = "";
	document.getElementById("pageCtrl_div").innerHTML += "<input type='text' id='validSlotCount' name='validSlotCount' value='0' >";  //插入卡的总数量
	document.getElementById("pageCtrl_div").innerHTML += "<input type='text' id='currentSlotID' name='currentSlotID' value='0' >";   //当前卡的ID信息

	//查询所有已插入卡的槽列表
	var currentSlotID = 0;				//当前操作的智能卡ID
	var slotList = Com11If.GetSlotList();  //查询所有已插入卡的槽列表
	if( null != slotList )
	{
		//修改槽列表显示信息
		document.getElementById("slotLinks_div").style.display = "";  //slotLinks_div：插入卡槽的卡名字
		document.getElementById("slotlabel").innerHTML = "已插入卡的槽列表";
		//将获取到的槽列表转换成数组，存储到全局变量g_slotIDs中
		var slotIDs = new VBArray( slotList ).toArray();  //将对象列表转化成VB数组
		var slotCount = slotIDs.length;	//已插入的智能卡数量
		var slotID = 0;
		//画槽信息隐藏域
		for( var i=0; i<slotCount; i++ )  //对卡进行遍历，获得有效卡id密码等一系列参数
		{
			slotID = slotIDs[i];
			//pageCtrl_div层 type='text' 
			document.getElementById("pageCtrl_div").innerHTML += "<input type='text' id='validSlotID_" + i + "' name='validSlotID_" + i + "' value='0' >";   //插入有效卡的Id
			//pageDetailCtrl_div层
			if( null == document.getElementById('isLogin_'+slotID) )
			{
				document.getElementById("pageDetailCtrl_div").innerHTML += "<input type='text' id='isLogin_" + slotID + "' name='isLogin_" + slotID + "' value='0' >";
		...
		if( slotCount > 0 )
		{
			...
			for( var i=0; i<slotCount; i++ )
			{
				...
				//将槽列表信息存入隐藏域
				var tokenInfo = Com11If.GetTokenInfo(slotID);   //获取令牌信息，返回令牌信息对象
				if( tokenInfo )
				{
					document.getElementById("validSlotID_" + i ).value = slotID;
					document.getElementById("maxPINLen_" + slotID).value = tokenInfo.GetMaxPinLen();  //获取密码最大长度
					document.getElementById("minPINLen_" + slotID).value = tokenInfo.GetMinPinLen();  //获取密码最小长度
				}
				
				//获取槽信息
				var slotInfo = Com11If.GetSlotInfo(slotID);  //根据令牌所在槽的ID获取令牌信息，返回槽信息对象
				if( slotInfo )
				{
					if( 1 == document.getElementById('isLogin_'+slotID).value )  //用户状态登录层的消息
					{
					...
					//初始化槽列表
					document.getElementById("slotLinks_div").innerHTML += "<a id='slotDesc_"+slotID+"' style='display:none;' href='#' onclick='showTokenInfo("+slotID+")' ></a>";  //date上面的卡槽令牌信息处理处的信息
					document.getElementById("slotLinks_div").innerHTML += "<a id='slotData_"+slotID+"' style='display:none;' href='#' onclick='showObjectList("+slotID+")'></a>";  //date处的信息
					document.getElementById("slotDesc_"+slotID).style.display = "";
					document.getElementById("slotDesc_"+slotID).innerHTML = slotInfo.GetSlotDescription();  //将描述放入到里面 （FT SCR2000A 0）
					document.getElementById("slotData_"+slotID).style.display = "";
					document.getElementById("slotData_"+slotID).innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;Data";  //显示date数据

					//初始化槽下拉列表
					var slotDescription = slotInfo.GetSlotDescription();  //每一个下拉列表的槽初始化select（FT SCR2000A 0）
					//初始化“用户登录”的槽下拉列表
					var login_objectItem = new Option(slotDescription, slotID);
					document.getElementById("login_slotSel").options.add(login_objectItem);  //把Id和卡槽的名称放到options中来
					//初始化“修改用户PIN码”的槽下拉列表
 ...
 * 清除槽列表层
*/
function clearSlotLinksDiv()  //这个方法是做清除用户状态上面已插入卡的槽列表
{
	//清除内容
 ...
 * 清除槽信息层  
*/
function clearSlotInfoDiv()  //用来清除槽信息列表信息
{
	//清除内容
 ...
 * 清除对象信息层
*/
function clearObjectInfoDiv()  //清除对象详细信息
{
	//清除内容
 ...
 * 清除槽下拉列表
*/
function clearSlotSel()  //清除各个功能模块的槽下拉列表的名字信息
{
	//清空“用户登录”的槽下拉列表
	var i=0;
 ...
 * 点击用户登录的“槽列表”
*/
function LoginSelectSlot()  //下拉槽列表
{
...
function Login()  //点击登录那几个字
{
	...
	//检查卡是否插入
	if( 0 == document.getElementById("validSlotCount").value )  //有效卡的数量 如果没有插卡的话，提示
	{
		alert("请插入卡！");
		return ;
	}

	if( "" == document.getElementById("login_div").style.display )  //如果已经打开，再点击就是合上了
	{
		document.getElementById("login_div").style.display = "none";
	}
	else
	{
		document.getElementById("login_div").style.display = "";  //否则，如果合上了，点击就是打开了
		/*
		//遍历卡的登录状态
...
function onLogin()
{
	...
	{
		//登录
		var slotID = document.getElementById("login_slotSel").value;  //选择id
		document.getElementById("currentSlotID").value = slotID;  //当前选择卡的ID
		var desc = document.getElementById("loginStatus_" + slotID).value;  //登录卡的那个名字
		var ret = Com11If.Login( slotID, PIN );  //登录调用的方法
		if( false == ret )
		{
			var count = Com11If.GetPINTryCounter( slotID, 1);  //获取密码验证重试次数
			if( 0 == count)  //剩余零次
			{
				alert("卡已经死锁！");
			}
			else if(0xFFFF == count)  //卡出异常了
			{
			    alert("Token异常！请重新插拔后重试！");
			}
			else
				alert("登录失败，你还有 "+count+"次机会！");
			document.getElementById("PIN").value = "";  //密码置空
			document.getElementById("PIN").focus();
			document.getElementById("isLogin_" + slotID).value = 0;
			//修改用户状态显示
		...
		{
			alert("登录成功！");
			document.getElementById("isLogin_" + slotID).value = 1;   //将登录状态更改为1
			//修改用户状态显示（已登录）
			document.getElementById("login_div").style.display = "none";   //合上
			document.getElementById("loginStatus_" + slotID).value = desc.substring(0, desc.indexOf("槽")) + "槽已登录";  //更改下拉框卡槽名状态
...
//	document.getElementById("slotData_"+slotID).style.display = "";
	...
	var objects = null;
	//查找数据对象(CKO_DATA)
	objects = Com11If.EnumObject( slotID, 0, 0 );   //枚举令牌对象
	if( null != objects )
	{
...
//		document.getElementById("CKO_PUBLIC_KEY").style.display = "";
	...
	if( 1 == document.getElementById("isLogin_" + slotID).value )  //判断登录状态，如果为已登录则可以看到私钥对象，如果没有登录则看不到私钥对象
	{
...
function selectObj(hobj)
{
	...
	{
		document.getElementById("deleteObj").disabled = true;  //这个话完全可以去掉，因为写这个input的时候已经加入了disabled这参数
	}
...
//dialogTop=250; dialogLeft=250;
	...
	//alert(currentSlotID);
	var str = window.showModalDialog('importObject.jsp', '', 'dialogHeight=220px; dialogWidth=290px; toolbar=no; titlebar=no; help=no; scrollbars=no; resizable=no; status=no;');
	if( "" != str)
	{
//		alert("str =" + str);
...
/*
 * 以十六进制显示点击证书后的结果，放在对应的textarea中
 */
...
/*
 * 显示对象属性列表   CKA_CLASS所有属性的结果
*/
function showAttributeList(attributes)
{
	//clearAttributeListShow();

	var n = attributes.GetItemCount();  //获取某属性对象列表中的属性数量
/*	if( n > 0 )
...
*/
	for( var i=0; i<n; i++ )
	{
		...
		{
			var attri_value = vb_attri.toArray();
			switch(attributes.GetAttributeType(i))  //获取属性类型
			{
			case 0x00000000:		//CKA_CLASS 
				document.getElementById("cka_0").style.display = ""; 
				document.getElementById("attri_len_0").innerHTML = attributes.GetAttributeLength(i);  //获取属性值长度
...
<div id="pagecell1_div">
	...
	<div id="pageNav_div">
	    ...
	    <div id="optionLinks_div">
			<a href="#">用户状态</a>
			<div id="userStatus_div" style="display:">
				<!-- 显示用户状态
